...
export default function ChangePasswordPage() {
  ...
  const getStrengthConfig = () => {
    if (strength <= 2)
      return {
        ...
        color: "bg-rose-500",
        text: "text-rose-500",
        progress: "bg-rose-500/20 [&>div]:bg-rose-500",
      };
    if (strength <= 3)
      return {
        label: "Cơ bản",
        color: "bg-amber-500",
        text: "text-amber-500",
        progress: "bg-amber-500/20 [&>div]:bg-amber-500",
      };
    if (strength <= 4)
      return {
        label: "An toàn",
        ...
        progress: "bg-emerald-500/20 [&>div]:bg-emerald-500",
      };
    return {
      label: "Rất mạnh",
      color: "bg-cyan-500",
      text: "text-cyan-500",
      progress: "bg-cyan-500/20 [&>div]:bg-cyan-500",
    };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        tMsg(res) ||
          "Mật khẩu đã được thay đổi an toàn! Hệ thống sẽ yêu cầu bạn đăng nhập lại.",
      );
  ...
  return (
    <main className="flex-1 flex flex-col px-6 overflow-hidden relative selection:bg-pink-500/30">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-8 py-6 border-b border-white/5 bg-background/50 backdrop-blur-md">
        ...
        <Badge
          variant="secondary"
          className="w-fit h-fit px-3 py-1.5 gap-2 border border-cyan-500/20 bg-cyan-500/5 text-cyan-600 dark:text-cyan-400 font-bold uppercase tracking-widest text-[9px] shadow-[0_0_10px_rgba(6,182,212,0.15)]"
        >
          <ShieldCheck size={14} />
                ...
                <CardHeader className="border-b dark:border-white/5 pb-8 pt-8 mx-8 px-0">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <CardDescription className="text-muted-foreground dark:text-slate-400 font-medium">
                ...
                <CardContent className="pt-8 px-8 space-y-8">
                      ...
                      <div className="p-4 rounded-xl border bg-muted/20 dark:bg-black/20 dark:border-white/5 shadow-inner space-y-3 transition-all duration-300">
                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                          <span className="text-muted-foreground dark:text-slate-500">
                            {t("auto.changePassword_10")}
                          </span>
                          <span
                            className={`transition-colors duration-300 ${strengthText}`}
                          >
                            {strengthLabel}
                          </span>
                        </div>
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    {confirmPassword && newPassword && (
                      <div
                        className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest mt-2 p-2 rounded-lg ${
                          confirmPassword === newPassword
            ...
            <div className="space-y-6">
              <Card className="shadow-2xl shadow-black/5 border-muted/60 dark:border-white/5 rounded-[2.5rem] dark:bg-[#0B1120]/60 dark:backdrop-blur-xl">
                ...
                <CardContent className="space-y-5 pt-8">
                  {[
                    {
                      text: "Sử dụng tối thiểu 8 ký tự",
                      icon: <CheckCircle2 size={16} />,
                    },
                    {
                      text: "Kết hợp chữ HOA, thường và số",
                      icon: <CheckCircle2 size={16} />,
                    },
                    {
                      text: "Thêm dấu cách ký tự đặc biệt",
                      icon: <CheckCircle2 size={16} />,
                    },
                    {
                      text: "Không dùng chung với app khác",
                      icon: <CheckCircle2 size={16} />,
                    },
                    {
                      text: "Thay đổi định kỳ 30 ngày/lần",
                      icon: <CheckCircle2 size={16} />,
                    },
                  ].map((tip, i) => (
                    <div key={i} className="flex items-start gap-4 group">
                      ...
                      <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground dark:text-slate-400 group-hover:text-foreground dark:group-hover:text-slate-200 transition-colors mt-0.5 leading-relaxed">
                        {tip.text}
                      </span>
                    </div>
                  ))}
              ...
              <Card className="border border-amber-500/20 bg-amber-500/5 shadow-2xl rounded-[2rem] overflow-hidden relative">
                ...
                <CardContent className="p-6 md:p-8 flex gap-5 relative z-10">
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-pink-600 dark:text-pink-300 uppercase tracking-widest drop-shadow-sm">
              ...
              <div className="p-5 rounded-2xl border bg-cyan-500/5 dark:bg-[#0B1120]/60 flex items-center gap-4 border-cyan-500/20 dark:backdrop-blur-xl shadow-lg shadow-black/5 mt-4 group cursor-pointer hover:-translate-y-1 transition-transform">
                ...
                <p className="text-[11px] text-muted-foreground dark:text-slate-400 font-bold uppercase tracking-wider">
                  ...
                  <Link
                    href="/support"
                    className="text-cyan-500 hover:text-cyan-400 ml-1 drop-shadow-sm"
                  >
