...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      console.log("🚀 [STEP 1] Calling sendOtpRegister with:", {
      ...
      // Validate + gửi OTP - gửi đầy đủ thông tin để BE validate trước
      const result = await sendOtpRegister({
        username: formData.username,
        email: formData.email,
      ...
      console.log("✅ [STEP 1] sendOtpRegister SUCCESS:", result);

      setStep("otp");
    } catch (err: any) {
      console.error("❌ [STEP 1] sendOtpRegister ERROR:", err);
      setServerError(
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      console.log("🚀 [STEP 2] Calling registerUser with OTP:", {
        username: formData.username,
        email: formData.email,
        otpCode: otpCode,
      });

      // Gửi đầy đủ thông tin + OTP lên đăng ký
      ...
      console.log("✅ [STEP 2] registerUser SUCCESS:", result);

      alert("Đăng ký thành công! Bạn có thể đăng nhập ngay.");
      router.push("/login");
    } catch (err: any) {
      console.error("❌ [STEP 2] registerUser ERROR:", err);
      setServerError(
      ...
      <div className="bg-card-bg/70 backdrop-blur-2xl border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden relative group ring-1 ring-white/5">
        ...
        <div className="px-8 pb-8">
          ...
          {step === "register" ? (
            ...
            >
              ...
              >
                {isRegistering ? "Đang xác thực..." : "Xác nhận OTP"}
              </button>
              <button
                type="button"
                onClick={() => setStep("register")}
        ...
        <div className="px-8 py-5 bg-slate-900/40 border-t border-white/5 text-center backdrop-blur-md">
          <p className="text-sm text-slate-400">
            Đã có tài khoản?
            <Link
              className="font-bold text-secondary hover:text-pink-400 hover:underline decoration-2 ml-2"
              href="/login"
            >
              Đăng nhập ngay
            </Link>
