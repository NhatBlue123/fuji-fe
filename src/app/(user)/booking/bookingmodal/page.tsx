...
export default function MySchedulePage() {
  ...
  const [actionType, setActionType] = useState<"CANCEL" | "END_EARLY">(
    "CANCEL",
  );
  ...
  const [cancelBooking, { isLoading: isCancelling }] =
    useCancelBookingMutation();
  const [endBookingSession, { isLoading: isEndingEarly }] =
    useEndBookingVideoSessionMutation();
      ...
      <div className="px-10 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
            {(["UPCOMING", "COMPLETED", "CANCELLED"] as BookingTab[]).map(
              (t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
          ...
          {items.map((c) => (
            ...
            >
              <div className="flex items-center gap-4 flex-1 w-full">
                <img
                  src={
                    (c.role === "TEACHER"
                      ? c.studentAvatarUrl
                      : c.teacherAvatarUrl) || "/images/avt-default.jpg"
                  }
              ...
              <div className="flex items-center gap-8 px-8 border-x border-white/10">
                ...
                <div className="flex flex-col items-center">
                  <p className="text-slate-500 text-xs uppercase tracking-wider">
                    {t("auto.bookingModal_9")}
                  </p>
                  <p className="text-white font-bold">
                    {formatTimeRange(c.startAt, c.endAt)}
                  </p>
                </div>
              </div>
