...
export default function SubscriptionPage() {
  ...
  const [toggleAutoRenew, { isLoading: isToggling }] =
    useToggleAutoRenewMutation();

  useEffect(() => setMounted(true), []);

  if (!mounted) return null;
  if (isUserLoading || isSubLoading || isHistoryLoading)
    return <LoadingState />;
  ...
  const handleToggleAutoRenew = async (checked: boolean) => {
    try {
      ...
      toast.success(
        tMsg(res) ||
          (checked ? "Đã bật gia hạn tự động" : "Đã tắt gia hạn tự động"),
      );
      refetch();
    } catch (err: any) {
      toast.error(
    ...
    <main className="flex-1 flex flex-col px-6 overflow-hidden relative selection:bg-pink-500/30">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-8 py-6 border-b border-white/5 bg-background/50 backdrop-blur-md">
        <div className="space-y-1">
          ...
          >
            <ArrowLeft size={14} />
            {t("auto.subscription_1")}
          </button>

          <Badge
            variant="secondary"
            className="px-3 py-1 gap-2 border-pink-500/20 bg-pink-500/5 text-pink-500 dark:text-pink-400 mb-2"
          >
            <Crown size={12} />
            {t("auto.subscription_2")}
          </Badge>
          <h1 className="text-4xl font-black tracking-tight uppercase">
            {t("auto.subscription_3")}
            <span
              className={`drop-shadow-[0_0_15px_rgba(236,72,153,0.3)] ${currentDetails.textColor}`}
            >
              {currentTier}
            </span>
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Star className="mr-2" size={18} strokeWidth={3} />
            {t("auto.subscription_5")}
          </Button>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8 animate-in fade-in duration-500">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <Card
            className={`lg:col-span-8 overflow-hidden border-none bg-gradient-to-br from-[#0B1120] via-[#111827] to-[#0a0c10] text-white relative shadow-2xl rounded-[2.5rem]`}
          >
            ...
            <div
              className={`absolute bottom-0 left-0 w-[300px] h-[300px] ${currentTier === "PREMIUM" ? "bg-purple-500/10" : "bg-cyan-500/10"} rounded-full blur-[80px] translate-y-1/2 -translate-x-1/2 pointer-events-none`}
            />
            ...
            <CardHeader className="relative z-10 pt-10 px-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-2xl bg-secondary hover:bg-secondary/90 ${currentDetails.color} flex items-center justify-center shadow-lg ${currentDetails.shadow}`}
                  >
                    {currentDetails.icon}
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-1">
                      {t("auto.subscription_6")}
                    </p>
                    <h3
                      className={`text-2xl font-black uppercase tracking-widest ${currentDetails.textColor}`}
                    >
                ...
                {mySub?.expireAt && (
                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-1">
                      {t("auto.subscription_7")}
                    </p>
                    <p className="font-mono text-lg font-bold">
                      {mySub.daysRemaining > 0
              ...
              {mySub && currentTier !== "BASIC" ? (
                <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white">
                          {t("auto.subscription_8")}
                        </p>
                        <p className="text-xs text-white/50">
                          {t("auto.subscription_9")}
                        </p>
                      </div>
                    </div>
                    <Switch
                  ...
                  {mySub.expireAt && (
                    <div className="flex items-center gap-2 text-xs text-white/60">
                      ...
                      <span>
                        Ngày hết hạn:{" "}
                        {new Date(mySub.expireAt).toLocaleDateString("vi-VN", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                        })}
                ...
                <div className="p-6 rounded-2xl bg-white/5 border border-white/10 text-center space-y-3">
                  ...
                  <p className="text-sm font-bold">
                    {t("auto.subscription_10")}
                  </p>
                  <p className="text-xs text-white/50">
                    {t("auto.subscription_11")}
                  </p>
                </div>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {mySub?.activeFeatures.map((feat, idx) => (
                      <div key={idx} className="flex items-start gap-2">
                        <CheckCircle2
                          size={16}
                          className={currentDetails.textColor}
                        />
          ...
          <div className="lg:col-span-4 space-y-4">
            <Card className="shadow-xl shadow-black/5 border-muted/60 dark:border-white/5 h-[140px] flex items-center justify-between px-8 transition-transform hover:-translate-y-1 rounded-[1.5rem] dark:bg-[#0B1120]/60 dark:backdrop-blur-xl group relative overflow-hidden">
              ...
              <div
                className={`w-14 h-14 rounded-2xl bg-secondary hover:bg-secondary/90 ${currentDetails.color} rotate-12 flex items-center justify-center shadow-lg text-white transition-transform duration-500 group-hover:rotate-0 relative z-10`}
              >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <CardTitle className="text-xl font-bold uppercase tracking-tight text-foreground dark:text-white">
                  {t("auto.subscription_14")}
                </CardTitle>
                <CardDescription className="text-muted-foreground dark:text-slate-400">
                  {t("auto.subscription_15")}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <Table>
              <TableHeader className="bg-muted/30 dark:bg-black/10 uppercase text-[10px] font-black tracking-widest text-muted-foreground dark:text-slate-500">
                <TableRow className="dark:border-white/5">
                  <TableHead className="px-8 h-12">
                    {t("auto.subscription_16")}
                  </TableHead>
                  <TableHead className="px-8 h-12">
                    {t("auto.subscription_17")}
                  </TableHead>
                  <TableHead className="px-8 h-12">
                    {t("auto.subscription_18")}
                  </TableHead>
                  <TableHead className="px-8 h-12 text-right">
                    {t("auto.subscription_19")}
                  </TableHead>
                  <TableHead className="px-8 h-12 text-right">
                    {t("auto.subscription_20")}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {history.map((tx: any) => (
                  <TableRow
                    key={tx.id}
                    className="hover:bg-muted/20 dark:hover:bg-white/[0.02] transition-colors group border-b dark:border-white/5 last:border-0"
                  >
                    ...
                    <TableCell className="px-8 py-6">
                      <span className="text-[10px] text-muted-foreground dark:text-slate-500 font-bold">
                        ...
                        {new Date(tx.endDate).toLocaleDateString("vi-VN")}
                      </span>
                    </TableCell>
                    <TableCell className="px-8 py-6 text-right font-black tracking-tighter text-foreground dark:text-white">
                      ...
                      <span className="text-[10px] ml-1 opacity-40">
                        {t("auto.subscription_21")}
                      </span>
                    </TableCell>
                    <TableCell className="px-8 py-6 text-right">
                      <Badge
                        variant="outline"
                        className={`px-3 py-1 text-[9px] font-black uppercase ring-0 shadow-inner border
                        ${
                          tx.status === "ACTIVE"
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="py-24 text-center">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm font-bold uppercase tracking-widest text-foreground dark:text-white">
                            {t("auto.subscription_22")}
                          </p>
                          <p className="text-[11px] text-muted-foreground dark:text-slate-500 mb-4 max-w-xs mx-auto">
                            {t("auto.subscription_23")}
                          </p>
                        </div>
                      </div>
                    </TableCell>
