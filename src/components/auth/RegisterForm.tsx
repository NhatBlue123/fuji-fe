...
export default function RegisterForm() {
    ...
    const validate = () => {
        ...
        if (!formData.fullname.trim()) {
            ...
            newErrors.fullname = 'Tên đăng nhập phải từ 3–20 ký tự';
        } else if (/\s/.test(formData.fullname)) {
            newErrors.fullname = 'Tên đăng nhập không được chứa khoảng trắng';
        } else if (!usernameRegex.test(formData.fullname)) {
            newErrors.fullname =
                'Chỉ cho phép chữ, số, dấu _ và . (không ở đầu/cuối)';
        }
