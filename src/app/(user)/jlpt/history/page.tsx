...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const LEVELS: (JLPTLevel | "Tất cả")[] = ["Tất cả", "N1", "N2", "N3", "N4", "N5"];
const TIME_FILTERS = [
...
export default function JlptHistoryPage() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<JLPTLevel | "Tất cả">("Tất cả");
  ...
  const filtered = useMemo(() => {
    ...
    return attempts
      .filter((a) => {
        if (selectedLevel !== "Tất cả" && a.test?.level !== selectedLevel) return false;
        if (selectedTime !== "all") {
  ...
  const stats = useMemo(() => {
    ...
    return { total, passed, passRate: Math.round((passed / total) * 100), best };
  }, [attempts]);
  ...
  const SortIcon = ({ field }: { field: "date" | "score" }) =>
    sortField !== field ? (
      ...
      <span className="material-symbols-outlined text-[14px] text-pink-400">arrow_downward</span>
    ) : (
      <span className="material-symbols-outlined text-[14px] text-pink-400">arrow_upward</span>
    );
    ...
    <div className="bg-[#0B1120] text-white px-6 md:px-12 lg:px-16 py-10 min-h-full">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Toàn bộ kết quả các lần làm bài của bạn</p>
        </div>
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Tổng lần thi", value: stats.total, icon: "assignment", color: "text-blue-400", bg: "bg-blue-500/10" },
              { label: "Đã đậu", value: stats.passed, icon: "verified", color: "text-emerald-400", bg: "bg-emerald-500/10" },
              { label: "Tỉ lệ đậu", value: `${stats.passRate}%`, icon: "percent", color: "text-yellow-400", bg: "bg-yellow-500/10" },
              { label: "Điểm cao nhất", value: stats.best, icon: "emoji_events", color: "text-pink-400", bg: "bg-pink-500/10" },
            ].map((s) => (
              <div key={s.label} className="rounded-2xl bg-slate-800/60 border border-white/5 p-5 flex items-center gap-4">
        ...
        {chartData.length >= 2 && (
          <div className="rounded-2xl bg-slate-800/60 border border-white/5 p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Biểu đồ tiến bộ</h2>
                <p className="text-xs text-slate-400">{chartData.length} lần thi gần nhất</p>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <YAxis domain={[0, 180]} tick={{ fill: "#94a3b8", fontSize: 11 }} />
                ...
                />
                <Line type="monotone" dataKey="Tổng điểm" stroke="#f472b6" strokeWidth={2.5} dot={{ r: 4, fill: "#f472b6" }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="Từ vựng/NGP" stroke="#60a5fa" strokeWidth={1.5} dot={{ r: 3 }} strokeDasharray="4 2" />
                <Line type="monotone" dataKey="Đọc hiểu" stroke="#34d399" strokeWidth={1.5} dot={{ r: 3 }} strokeDasharray="4 2" />
                <Line type="monotone" dataKey="Nghe hiểu" stroke="#a78bfa" strokeWidth={1.5} dot={{ r: 3 }} strokeDasharray="4 2" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="rounded-2xl bg-slate-800/60 border border-white/5 overflow-hidden">
          <div className="px-6 py-4 border-b border-white/5 flex items-center gap-2">
            ...
            <span className="text-sm font-normal text-slate-400">({filtered.length} kết quả)</span>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/5 text-slate-400 text-xs uppercase tracking-wide">
                    <th className="text-left px-6 py-3 font-semibold">Đề thi</th>
                    ...
                    <th className="text-center px-4 py-3 font-semibold hidden lg:table-cell">Từ vựng/NGP</th>
                    <th className="text-center px-4 py-3 font-semibold hidden lg:table-cell">Đọc hiểu</th>
                    <th className="text-center px-4 py-3 font-semibold hidden lg:table-cell">Nghe hiểu</th>
                    <th className="text-center px-4 py-3 font-semibold hidden sm:table-cell">Thời gian</th>
                    <th
                      className="text-center px-4 py-3 font-semibold cursor-pointer hover:text-white transition-colors select-none"
                    ...
                    <th className="text-center px-4 py-3 font-semibold">Kết quả</th>
                    ...
                    >
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-slate-400 mt-0.5">
                          {attempt.totalQuestions} câu • {attempt.test?.duration ?? "--"} phút
                        </div>
                      </td>
                      ...
                      <td className="text-center px-4 py-4">
                        ...
                        <span className="text-xs text-slate-500">/{attempt.test?.maxScore ?? 180}</span>
                      </td>
