...
export default function CoursesPage() {
  ...
  const [sortBy, setSortBy] = useState<"newest" | "oldest" | "students" | "rating" | "price">("newest");
  ...
  const overview = useMemo(() => {
    ...
    const totalStudents = data.content.reduce((sum, c) => sum + c.studentCount, 0);
    const totalDuration = data.content.reduce((sum, c) => sum + c.totalDuration, 0);
  ...
  const handleTogglePublish = async (course: (typeof data.content)[number]) => {
    ...
    try {
      ...
      toast.success(course.isPublished ? "Đã chuyển về bản nháp" : "Đã xuất bản khóa học");
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Toàn bộ trong hệ thống</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <CardTitle className="text-3xl">{overview.publishedCount}</CardTitle>
            <p className="text-xs text-muted-foreground mt-1">{overview.draftCount} bản nháp (trang hiện tại)</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <CardTitle className="text-3xl">{overview.totalStudents.toLocaleString()}</CardTitle>
            <p className="text-xs text-muted-foreground mt-1">Cộng dồn ở dữ liệu hiện đang tải</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">phút / khóa (trang hiện tại)</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base font-semibold">Tổng quan trạng thái</CardTitle>
          <CardDescription>Phân bố xuất bản và bản nháp theo dữ liệu hiện tại</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {[{ label: "Đã xuất bản", value: overview.publishedCount }, { label: "Bản nháp", value: overview.draftCount }].map((item) => {
              const total = Math.max(overview.publishedCount + overview.draftCount, 1);
              const pct = Math.round((item.value / total) * 100);
              return (
                <div key={item.label} className="flex items-center gap-3">
                  <span className="w-24 text-sm font-medium text-muted-foreground">{item.label}</span>
                  ...
                  <span className="w-20 text-right text-sm text-muted-foreground">{item.value} ({pct}%)</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-3">
          ...
          <div className="w-[220px]">
            <Select
              value={sortBy}
              onValueChange={(v: "newest" | "oldest" | "students" | "rating" | "price") => {
                setSortBy(v);
                setCurrentPage(1);
              }}
            >
      ...
      {!isLoading && !isError && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Quản lý nhanh khóa học theo bộ lọc hiện tại</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="min-w-[180px]">Giảng viên</TableHead>
                    <TableHead className="text-right">Giá</TableHead>
                    <TableHead className="text-center">Trạng thái</TableHead>
                    <TableHead className="text-center min-w-[180px]">Chỉ số</TableHead>
