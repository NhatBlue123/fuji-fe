...
function statusClass(status?: VoiceTranscriptStatus, enabled = true): string {
  if (!enabled) return "border-border bg-muted text-muted-foreground dark:border-white/10 dark:bg-white/[0.04] dark:text-[#8B8FA8]";
  if (status === "active") return "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-400/10 dark:text-emerald-300";
  if (status === "error") return "border-[#FF6B6B]/30 bg-[#FF6B6B]/10 text-[#FF9B9B]";
  if (status === "connecting" || status === "reconnecting") {
    return "border-[#6C63FF]/30 bg-[#6C63FF]/10 text-[#B9B4FF]";
  }
  return "border-border bg-muted text-muted-foreground dark:border-white/10 dark:bg-white/[0.04] dark:text-[#8B8FA8]";
}
...
export function TranscriptPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-card text-foreground dark:bg-[#1e2130] dark:text-[#F0F0F0]">
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-border px-4 dark:border-white/[0.08]">
      ...
      <div className="shrink-0 border-b border-border px-4 py-3 dark:border-white/[0.08]">
        <div className="flex items-center gap-2 rounded-xl border border-input bg-background px-3 py-2 dark:border-white/10 dark:bg-[#171a24]">
          <Search className="h-4 w-4 shrink-0 text-muted-foreground dark:text-[#8B8FA8]" />
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Search transcript"
            className="min-w-0 flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground dark:text-[#F0F0F0] dark:placeholder:text-[#8B8FA8]"
          />
        </div>
        {(error || voiceError) && (
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto px-4 py-3">
        {filteredTranscripts.length === 0 && !partialText.trim() ? (
          <div className="flex h-full flex-col items-center justify-center px-4 text-center">
            <p className="text-sm font-medium text-foreground dark:text-[#F0F0F0]">
              {enabled ? "Chưa có transcript lời nói" : "Transcript đang tắt"}
            </p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground dark:text-[#8B8FA8]">
              {enabled
          ...
          <div className="space-y-3">
            {filteredTranscripts.map((item) => {
              const speaker = item.speakerName || roleLabel(item.speakerRole);
              return (
                <div key={transcriptKeyForRender(item)} className="rounded-xl bg-muted px-3 py-2.5 dark:bg-[#171a24]">
                  <div className="mb-1 flex items-center justify-between gap-2">
                    <span className="truncate text-xs font-semibold text-foreground dark:text-[#D8D9E8]">{speaker}</span>
                    <span className="shrink-0 text-[11px] text-muted-foreground dark:text-[#8B8FA8]">{formatTranscriptTime(item)}</span>
                  </div>
                  <p className="whitespace-pre-wrap break-words text-sm leading-6 text-foreground dark:text-[#F0F0F0]">{item.content}</p>
                </div>
              );
            })}
            ...
            {partialText.trim() && (
              <div className="rounded-xl border border-[#6C63FF]/30 bg-[#6C63FF]/10 px-3 py-2.5">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <span className="truncate text-xs font-semibold text-foreground dark:text-[#D8D9E8]">
                    {currentUserName || roleLabel(currentUserRole)}
                  </span>
                  <span className="text-[11px] text-[#B9B4FF]">Đang nói</span>
                </div>
                <p className="whitespace-pre-wrap break-words text-sm leading-6 text-foreground dark:text-[#F0F0F0]">{partialText}</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="shrink-0 border-t border-border px-4 py-3 dark:border-white/[0.08]">
        <button
          type="button"
          onClick={handleSave}
          ...
          className={cn(
            ...
            transcripts.length > 0
              ? "border-border bg-muted text-foreground hover:bg-muted/80 dark:border-white/15 dark:bg-white/[0.04] dark:text-[#F0F0F0] dark:hover:bg-white/[0.08]"
              : "cursor-not-allowed border-border text-muted-foreground/50 dark:border-white/10 dark:text-[#8B8FA8]/50"
          )}
        >
