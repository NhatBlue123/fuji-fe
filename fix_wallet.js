...
let content = fs.readFileSync('src/app/(user)/profile/wallet/page.tsx', 'utf8');

// Chunk 1
const c1_target = `        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
...
const c1_rep = `        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Main Wallet Card - Đổi gradient nền sang tông hồng/purle mờ */}
          <Card className="lg:col-span-8 overflow-hidden border-none bg-gradient-to-br from-[#0B1120] via-[#111827] to-[#0a0c10] text-white relative shadow-2xl rounded-[2.5rem]">
            {/* Đổi Glow hồng */}
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-pink-500/10 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
...
const pattern = /<TableBody>\s*\{transactions\.map\(\(tx: Transaction\) => \(\s*<TableRow key=\{tx\.id\} className="hover:bg-muted\/20[\s\S]*?<\/TableRow>\s*<\/TableHeader>\s*<TableBody>/;

content = content.replace(pattern, "<TableBody>");

fs.writeFileSync('src/app/(user)/profile/wallet/page.tsx', content, 'utf8');
