...
export default function ProfilePage() {
  ...
  const { data: mySub, isLoading: isSubLoading } = useGetMySubscriptionQuery(
    undefined,
    {
      skip: isLoading || !user,
    },
    ...
    <div className="flex-1 overflow-y-auto relative scroll-smooth selection:bg-pink-500/30">
      ...
      <div className="relative w-full h-[320px] flex flex-col justify-center overflow-hidden rounded-b-[2.5rem] shadow-2xl shadow-pink-500/5 bg-background border-b border-border/50">
        <div className="absolute inset-0 z-0">
          ...
          <div
            className="w-full h-full bg-cover bg-bottom opacity-20 dark:opacity-30 mix-blend-overlay"
            style={{
              backgroundImage:
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full px-6 md:px-12 lg:px-20 -mt-10 text-center md:text-left">
          <Badge
            variant="secondary"
            className="px-3 py-1 gap-2 border-pink-500/20 bg-pink-500/10 text-pink-600 dark:text-pink-400 mb-4 inline-flex"
          >
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
              ...
              <div className="flex-1 space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                    ...
                    <Link href="/profile/subscription">
                      {displayTier === "PREMIUM" ? (
                        ...
                        <Badge
                          variant="outline"
                          className="border-muted dark:border-white/10 text-muted-foreground dark:text-slate-400 font-black uppercase tracking-widest text-[10px] px-3 py-1 hover:bg-muted/50 dark:hover:bg-white/5 transition-all cursor-pointer hover:border-pink-500/30 hover:text-pink-500 dark:hover:text-pink-400 active:scale-95"
                        >
                          Basic
                        </Badge>
                      )}
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-xl text-sm leading-relaxed italic border-l-2 border-pink-500/30 pl-4 py-1">
                  "
                  {user.bio ||
                    "Chưa có lời tựa cho bản thân... Hãy cập nhật hồ sơ để chia sẻ thêm về bạn."}
                  "
                </p>
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-4">
                  <Button
                    asChild
                    className="rounded-2xl h-11 px-6 bg-secondary hover:bg-secondary/90 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-pink-500/20 transition-all"
                  >
                  ...
                  <Button
                    ...
                    className="rounded-2xl h-11 px-6 bg-muted/40 dark:bg-white/5 border-muted dark:border-white/10 text-foreground dark:text-white hover:bg-rose-500/10 hover:text-rose-500 dark:hover:text-rose-400 hover:border-rose-500/30 font-black uppercase tracking-widest text-[10px] transition-all"
                  >
              ...
              <div className="bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-muted dark:border-white/10 p-6 rounded-3xl text-center space-y-3 min-w-[200px] shadow-inner shadow-black/5 dark:shadow-none shrink-0 lg:ml-auto">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-pink-600/80 dark:text-pink-100/60">
                    Hạng học tập
                  </p>
                  <p className="text-xl font-black text-foreground dark:text-white tracking-tighter uppercase drop-shadow-md">
                    Silver II
                  </p>
                </div>
                ...
                <p className="text-[10px] font-black text-pink-600/70 dark:text-pink-100/50 uppercase tracking-widest">
                  350 / 500 EXP
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-muted-foreground dark:text-slate-400">
                      Liên hệ và trình độ ngôn ngữ của bạn
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="pt-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <InfoItem
                    ...
                    label="Trình độ"
                    value={`Tiếng Nhật - ${user.jlptLevel || "Chưa xác định"}`}
                    color="pink text-blue-500 dark:text-blue-400"
                  />
                </div>
              </CardContent>
            </Card>
          ...
          <div className="fixed inset-0 bg-background/80 dark:bg-[#0B1120]/80 backdrop-blur-md flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
            <Card className="max-w-sm w-full border-muted/60 dark:border-white/10 shadow-2xl overflow-hidden rounded-[2.5rem] bg-background dark:bg-[#111827]">
              ...
              <CardHeader className="text-center pt-10 pb-4 relative z-10">
                ...
                <CardTitle className="text-xl font-black uppercase tracking-tight text-foreground dark:text-white">
                  Bạn muốn đăng xuất?
                </CardTitle>
                <CardDescription className="text-xs font-medium mt-2">
                  Hành động này sẽ kết thúc phiên làm việc hiện tại của bạn.
                </CardDescription>
              </CardHeader>
              <CardFooter className="flex gap-3 p-8 pt-4 relative z-10">
                <Button
                  ...
                  className="flex-1 h-12 rounded-xl border-muted dark:border-white/10 font-black uppercase text-[10px] tracking-widest text-foreground dark:text-white dark:hover:bg-white/10"
                >
                  Hủy
                </Button>
                <Button
                  onClick={handleLogout}
                  className="flex-1 h-12 rounded-xl bg-rose-500 hover:bg-rose-600 text-white font-black uppercase text-[10px] tracking-widest shadow-lg shadow-rose-500/20"
                >
                  Xác nhận
                </Button>
              </CardFooter>
...
function InfoItem({
  ...
  return (
    <div className="flex items-start gap-4 group">
      <div
        className={`p-3 rounded-2xl transition-transform duration-300 border shadow-inner shrink-0 group-hover:-translate-y-1 ${getColors()}`}
      >
        {icon}
      </div>
      <div className="space-y-1">
        <p className="text-[10px] font-black text-muted-foreground dark:text-slate-500 uppercase tracking-widest">
          {label}
        </p>
        <p className="font-bold text-sm tracking-tight text-foreground dark:text-slate-200">
          {value}
        </p>
      </div>
    </div>
...
function StatItem({
  ...
  return (
    <div className="flex items-center justify-between group">
      <div className="flex items-center gap-4">
        <div
          className={`p-2.5 rounded-xl border transition-transform group-hover:scale-110 shadow-inner ${getColors()}`}
        >
          {icon}
        </div>
      ...
      <span className="text-xl font-black text-foreground dark:text-white tracking-tighter">
        {value}
      </span>
    </div>
  );
  ...
  return (
    <Link href={href} className="group">
      <Card className="bg-white/60 dark:bg-[#0B1120]/60 backdrop-blur-xl shadow-xl shadow-black/5 border-muted/60 dark:border-white/5 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl cursor-pointer h-full relative overflow-hidden rounded-[2rem]">
        <div
          className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-[40px] transition-colors duration-500 ${c.glow}`}
        />
        <CardContent className="p-8 flex items-center gap-6">
          <div
            className={`p-4 rounded-2xl border shadow-inner transition-transform duration-500 group-hover:scale-110 shrink-0 ${c.iconContainer}`}
          >
            {icon}
          </div>
          <div className="flex-1 space-y-1 z-10">
            <h4
              className={`font-bold text-sm uppercase tracking-tight text-foreground dark:text-white transition-colors ${c.titleHover}`}
            >
              {title}
            </h4>
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-400 uppercase tracking-widest">
              {desc}
            </p>
          </div>
        </CardContent>
