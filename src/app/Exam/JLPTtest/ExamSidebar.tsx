    ...
    <aside className="w-80 shrink-0 border-l border-slate-700 bg-[#1E293B] hidden xl:flex flex-col z-30 h-full shadow-xl">
      ...
      <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
        <div className="flex items-center justify-between gap-2 mb-6 text-[10px] font-medium text-slate-400 bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
          ...
          <div className="flex items-center gap-1.5">
            <div className="size-2.5 rounded-full bg-[#ee2b5b]"></div>
            <span>Đã làm</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="size-2.5 rounded-full border-2 border-[#3b82f6]"></div>
            <span>Hiện tại</span>
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-lg flex items-center justify-center text-sm font-medium transition-all duration-200 relative overflow-hidden
                    ${
                      isCurrent
                        ? "z-10 font-bold text-white scale-110 shadow-lg"
                        : "text-slate-300 hover:opacity-100 hover:bg-slate-700"
                    }`}
                  style={{
      ...
      <div className="p-4 border-t border-slate-700 bg-[#162032]">
        <button
          ...
          onMouseOver={(e) =>
            !isCurrentFlagged && (e.currentTarget.style.backgroundColor = "#334155")
          }
          onMouseOut={(e) =>
            !isCurrentFlagged && (e.currentTarget.style.backgroundColor = "transparent")
          }
        >
          <span
            className={`material-symbols-outlined text-[20px] transition-colors ${
              isCurrentFlagged ? "text-yellow-400" : "group-hover:text-yellow-400"
            }`}
