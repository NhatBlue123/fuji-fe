        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            videoElement.play().catch(() => {
              // Náº¿u autoplay bá»‹ cháº·n, thá»­ phĂ¡t muted
        ...
        <section className="grid gap-8 lg:grid-cols-[0.95fr_1.35fr] lg:items-end">
          <FadeUp className="space-y-6">
            <SectionIntro
              ...
              title={T("home.featureMap.title").replace("má»™t nÆ¡i", "").replace("ä¸€ă‹æ‰€ă«", "")}
              highlight={
                T("home.featureMap.title").includes("má»™t nÆ¡i")
                  ? "má»™t nÆ¡i"
                  : T("home.featureMap.title").includes("ä¸€ă‹æ‰€")
                    ? "ä¸€ă‹æ‰€ă«"
                    : undefined
              }
              ...
              {JLPT_META.map((item) => (
                ...
                >
                  ...
                  <p className="mt-1 text-[11px] opacity-75">
                    {item.tests} Ä‘á» / {item.questions} cĂ¢u
                  </p>
                </Link>
              ))}
            </div>
            ...
            <div className="group overflow-hidden rounded-[1.75rem] border border-border/70 bg-card shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg dark:border-white/10 dark:bg-slate-950/35">
              <div className="relative h-[310px] overflow-hidden rounded-none bg-muted/50">
                ...
                <button
                  ...
                  aria-label={isAiVideoMuted ? "Báº­t Ă¢m thanh" : "Táº¯t Ă¢m thanh"}
                >
