...
export default function SubscriptionPage() {
  ...
  const handleToggleAutoRenew = async (checked: boolean) => {
    try {
      ...
      toast.success(tMsg(res) || (checked ? "Da bat gia han tu dong" : "Da tat gia han tu dong"));
      refetch();
    } catch (err: unknown) {
      const apiError = err as { data?: { messageKey?: string } };
      toast.error(tMsg(apiError.data?.messageKey) || tMsg("api.error") || "Loi cap nhat tu dong gia han");
    }
  };
    ...
    <main className="flex-1 flex flex-col px-6 overflow-hidden relative selection:bg-pink-500/30">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-8 py-6 border-b border-white/5 bg-background/50 backdrop-blur-md">
        <div className="space-y-1">
          ...
          >
            <ArrowLeft size={14} />{t("auto.subscription_1")}
          </button>

          <Badge variant="secondary" className="px-3 py-1 gap-2 border-pink-500/20 bg-pink-500/5 text-pink-500 dark:text-pink-400 mb-2">
            <Crown size={12} />{t("auto.subscription_2")}
          </Badge>
          <h1 className="text-4xl font-black tracking-tight uppercase">
            {t("auto.subscription_3")}
            <span className={`drop-shadow-[0_0_15px_rgba(236,72,153,0.3)] ${currentDetails.textColor}`}>{currentTier}</span>
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Star className="mr-2" size={18} strokeWidth={3} />{t("auto.subscription_5")}
          </Button>
        </div>
      </header>
                ...
                {mySub?.expireAt && (
                  <div className="text-right">
                    ...
                    <p className="font-mono text-lg font-bold">
                      {mySub.daysRemaining > 0 ? `Con ${mySub.daysRemaining} ngay` : "Het han"}
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-6">
                  ...
                  {mySub.expireAt && (
                    <div className="flex items-center gap-2 text-xs text-white/60">
                      <Calendar size={14} />
                      <span>Ngay het han: {new Date(mySub.expireAt).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric" })}</span>
                    </div>
                  )}
                </div>
              ) : (
                ...
                {history.map((tx) => (
                  <TableRow key={tx.id} className="hover:bg-muted/20 dark:hover:bg-white/[0.02] transition-colors group border-b dark:border-white/5 last:border-0">
                    ...
                    <TableCell className="px-8 py-6 text-right">
                      <Badge variant="outline" className={`px-3 py-1 text-[9px] font-black uppercase ring-0 shadow-inner border ${tx.status === "ACTIVE" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : tx.status === "EXPIRED" ? "bg-slate-500/10 text-slate-500 border-slate-500/20" : "bg-rose-500/10 text-rose-500 border-rose-500/20"}`}>
                        {tx.status}
                      </Badge>
                    </TableCell>
                  </TableRow>
