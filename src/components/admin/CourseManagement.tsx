...
export default function CourseManagement() {
  ...
  const handleAddCourse = (courseData: Omit<Course, 'id'>) => {
    if (editingCourse) {
      setCourses(prev =>
        prev.map(c => c.id === editingCourse.id ? { ...courseData, id: c.id } as Course : c)
      )
      setEditingCourse(null)
    } else {
      setCourses(prev => [
        { ...courseData, id: Date.now() } as Course,
        ...prev
      ])
    }
    setOpenModal(false)
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-blue-500 text-white hover:bg-blue-600 hover:shadow-md hover:shadow-blue-500/30 h-10 px-4 py-2"
          >
